...
def clean_text(t):
    ...
    text = re.sub(r'(\s)+|&Nbsp;', r' ', text)
    return text.strip()

def dict_to_row(p):
    row = {
        'Name': clean_text(p['name']),
        'Long Description': clean_text(p['long_description']),
        'Short Description': clean_text(p['short_description'])
    }
    return row
...
def clean_contractions(text, remove_stopwords=True):
